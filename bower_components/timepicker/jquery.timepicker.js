( function ( $ ) {
	...
	Timepicker.prototype = {
		...
		_init: function() {
			...
			this.$picker.prependTo( $(this.$el).parent() );
			this.$picker.show;
		...
		_parseTime: function( time ) {
			var matches = ( time || this.$el.val() ).match( /^(\d{2}):(\d{2}):(\d{2})$/ ),
				hours, minutes, sec, am;

			if ( matches ) {
				hours = parseInt( matches[1], 10 );
				minutes = parseInt( matches[2], 10 );
				sec = parseInt( matches[2], 10 );
				if ( hours < 24 && minutes < 60 && sec < 60 ) {
					am = hours < 12;
		...
		_composeTime: function( time ) {
			time = time || this.time;
			$(this.$el).attr('value', zeroise( time.hours + ( time.am ? 0 : 12 ) ) + ':' + zeroise( time.minutes )+ ':' + zeroise( time.sec ));
			return zeroise( time.hours + ( time.am ? 0 : 12 ) ) + ':' + zeroise( time.minutes )+ ':' + zeroise( time.sec );
		},
	...
	$.fn.timepicker = function( method ) {
		return this.each( function() {
			...
			if ( data ) {
				...
				// console.log($el.data( namespace, new Timepicker( $el ) ))
			}
		} );
	};
} )( window.jQuery );
